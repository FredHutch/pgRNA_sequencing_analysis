## rhino module: R/3.6.2-foss-2018b-fh1 # recommended by scicomp
...
import_library("Rsamtools") # new version = 1.34.1
import_library("tidyverse") # new version = 1.2.1
...
# saveRDS(test1, file.path(basedir, "bowtie1_aligned/200722_HeLa_screen/bam_sorted/test_case","test1.bam"))
# saveRDS(test2, file.path(basedir, "bowtie1_aligned/200722_HeLa_screen/bam_sorted/test_case","test2.bam"))
# d.bam.1 = readRDS("test1.bam") %>% as_tibble()
# d.bam.2 = readRDS("test2.bam") %>% as_tibble()
...
##   d.bam.1.sub = tibble("qname" = d.bam.1.sub[[1]]$qname, "rname" = d.bam.1.sub[[1]]$rname)
##   d.bam.1.sub <- d.bam.1.sub %>% mutate_at("rname", as.character)
